...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (bookinglistdata.bookings) {
            ...
            setDailygross(parseFloat(tdTrans).toFixed(0));
            setMonthlygross(parseFloat(mnTrans).toFixed(0));
            setTotalgross(parseFloat(totTrans).toFixed(0));
        }
        ...
        bookinglistdata.loading || usersdata.loading ? <CircularLoading/> :
            <div>
                ...
                <Grid container direction="row" spacing={2}>
                    <Grid item xs>
                        <DashboardCard title={language.today_text}
                                       image={require("../assets/img/money1.jpg")}>{formatPrice(dailygross)}</DashboardCard>
                    </Grid>
                    <Grid item xs>
                        <DashboardCard title={language.this_month_text}
                                       image={require("../assets/img/money2.jpg")}>{formatPrice(monthlygross)}</DashboardCard>
                    </Grid>
                    <Grid item xs>
                        <DashboardCard title={language.total}
                                       image={require("../assets/img/money3.jpg")}>{formatPrice(totalgross)}</DashboardCard>
                    </Grid>
                </Grid>
                {mylocation ?
                    <Paper style={{marginTop: '25px'}}>
                        <Typography variant="h4"
                                    style={{margin: "20px 0 0 15px"}}>{language.real_time_driver_section_text}</Typography>
                    ...
                    <Typography variant="h6" style={{
                        ...
                        color: '#FF0000'
                    }}>{language.allow_location}</Typography>
                }
            </div>
