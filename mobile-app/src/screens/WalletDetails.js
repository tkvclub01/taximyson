...
export default function WalletDetails(props) {
    ...
    const doWithdraw = () => {
        if (auth.info.profile.walletBalance > 100000) {
            props.navigation.push('withdrawMoney', {userdata: auth.info.profile});
        } else {
            Alert.alert(language.alert, language.wallet_zero_100)
        }
    }
    ...
    return (
        <View style={styles.mainView}>
            <Header
                ...
                rightComponent={auth.info && auth.info.profile ? <TouchableOpacity onPress={doWithdraw}><Text
                    style={{color: colors.WHITE}}>{language.withdraw}</Text></TouchableOpacity> : null}
                ...
                <View style={{height: walletBar, marginBottom: 12}}>
                    ...
                    <View style={{marginVertical: 10}}>
                        <Text style={{
                            ...
                            fontWeight: '500',
                            marginTop: 8
                        }}>{language.transaction_history_title}</Text>
                    </View>
                </View>
